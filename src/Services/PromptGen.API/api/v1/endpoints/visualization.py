...
from api.v1.dependencies import get_redis_cache, get_database  # <-- ИСПРАВЛЕНО
...
@router.post(
    ...
    response_model=SuccessResponse[CharacterConsistencyResponse],
    summary="Save character consistency data",
    description="Сохранить данные консистентности персонажа"
)
async def save_character_consistency(
    request: CharacterConsistencyRequest,
    db: AsyncSession = Depends(get_database),
    cache = Depends(get_redis_cache)
):
    """
    Сохранить данные консистентности персонажа.
