...
/// - Search, Popular, Preview, Health: Anonymous (public access)
/// - Single book import: Any authenticated user
...
public class ImportController : ControllerBase
{
    ...
    #region Import Operations (Require Authentication)
    ...
    /// <returns>Import result with book details</returns>
    /// <response code="200">Book imported successfully (already existed)</response>
    /// <response code="201">Book imported successfully (newly created)</response>
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInformation(
                "User {UserId} importing Gutenberg book {GutenbergId}",
                _currentUserService.UserId, gutenbergId);
            ...
            {
                _logger.LogWarning(
                    "Failed to import Gutenberg book {GutenbergId}: {Errors}",
                    gutenbergId, string.Join(", ", result.Errors.Select(e => e.Message)));
            ...
            _logger.LogInformation(
                "Successfully imported Gutenberg book {GutenbergId} as {BookId} ('{Title}')",
                gutenbergId, result.Value.BookId, result.Value.Title);
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Bulk import results with success/failure counts</returns>
    /// <response code="200">Bulk import completed</response>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogInformation(
                "Admin {UserId} starting bulk import from Gutenberg. IDs count: {IdsCount}, Max books: {MaxBooks}",
                _currentUserService.UserId,
                request.GutenbergIds?.Count ?? 0,
                request.MaxBooks);
            ...
            _logger.LogInformation(
                "Bulk import completed: {SuccessCount} succeeded, {FailedCount} failed, {SkippedCount} skipped",
                result.Value.SuccessCount, result.Value.FailedCount, result.Value.SkippedCount);
    ...
    #region Search & Browse Operations (Public Access)
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Search results from Gutenberg</returns>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogDebug(
                "Searching Gutenberg: Query='{Query}', Topic='{Topic}', Languages='{Languages}', Page={Page}",
                request.Search, request.Topic, request.Languages, request.Page);

            var criteria = new GutenbergSearchCriteriaDto
            {
                Search = request.Search,
                Languages = request.Languages?.Split(',', StringSplitOptions.RemoveEmptyEntries).ToList(),
                Topic = request.Topic,
            ...
            if (result.IsFailed)
            {
                _logger.LogWarning("Gutenberg search failed: {Error}", result.Errors.FirstOrDefault()?.Message);
    ...
    /// <param name="language">Filter by language code (e.g., 'en', 'fr')</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Popular books from Gutenberg</returns>
    /// <response code="200">Popular books retrieved successfully</response>
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogDebug("Getting popular Gutenberg books: Page={Page}, Language={Language}", page, language);
        ...
        catch (Exception ex)
        {
            ...
            {
                Success = false,
                Message = "An error occurred while fetching popular books"
            });
        }
    }
    ...
    /// <param name="gutenbergId">Project Gutenberg book ID</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Book details from Gutenberg</returns>
    /// <response code="200">Book preview retrieved successfully</response>
    /// <response code="404">Book not found in Gutenberg</response>
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogDebug("Getting Gutenberg book preview: {GutenbergId}", gutenbergId);
        ...
        catch (Exception ex)
        {
            ...
            {
                Success = false,
                Message = "An error occurred while fetching book preview"
            });
        }
    }
    ...
    [AllowAnonymous]
    [ProducesResponseType(typeof(ApiResponse<GutendexHealthResponse>), StatusCodes.Status200OK)]
    public async Task<IActionResult> CheckGutendexHealth(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var response = new GutendexHealthResponse
            {
            ...
            return Ok(new ApiResponse<GutendexHealthResponse>
            {
                Success = true,
                Data = response,
        ...
        {
            _logger.LogError(ex, "Error checking Gutendex health");

            return Ok(new ApiResponse<GutendexHealthResponse>
            {
                Success = true,
                Data = new GutendexHealthResponse
                {
                    IsAvailable = false,
                    LatencyMs = -1,
                    CheckedAt = DateTime.UtcNow,
                ...
                Message = "Gutendex service health check failed"
            });
        }
    }
...
/// <summary>
/// Response model for Gutendex health check
/// </summary>
public class GutendexHealthResponse
{
    /// <summary>
    /// Whether the Gutendex service is available
    /// </summary>
