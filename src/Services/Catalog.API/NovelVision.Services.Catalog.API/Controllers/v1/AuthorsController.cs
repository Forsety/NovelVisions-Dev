...
public class AuthorsController : ControllerBase
{
    ...
    [Authorize]
    [SwaggerOperation(Summary = "Become an author", Description = "Upgrades current user to Author role")]
    [SwaggerResponse(200, "Successfully upgraded to Author")]
    [SwaggerResponse(400, "Already an author or upgrade failed")]
    ...
    public async Task<IActionResult> BecomeAuthor()
    {
        ...
        // Check current role
        var currentUser = await _identityService.GetUserByIdAsync(userId);
        if (currentUser == null)
        {
            return NotFound(new { error = "User not found" });
        }

        // Check if already has Author or higher role (Roles is a collection)
        var hasAuthorRole = currentUser.Roles.Any(r =>
            r == "Author" || r == "Admin" || r == "SuperAdmin" || r == "Editor");

        if (hasAuthorRole)
        {
            return BadRequest(new { error = "You are already an author or have higher privileges" });
        }

        // Upgrade to Author
        var success = await _identityService.AssignRoleAsync(userId, "Author");

        if (!success)
        {
            _logger.LogWarning("Failed to upgrade user {UserId} to Author role", userId);
            return BadRequest(new { error = "Failed to upgrade to Author. Please try again." });
        }

        _logger.LogInformation("User {UserId} ({Email}) upgraded to Author role", userId, currentUser.Email);
        ...
        {
            success = true,
            message = "Congratulations! You are now an author. You can create and publish books.",
            user = updatedUser
        });
    }
