...
public class BookRepository : RepositoryBase<Book>, IBookRepository
{
    ...
    public async Task<Book?> GetByIdWithChaptersAndPagesAsync(BookId id, CancellationToken cancellationToken = default)
    {
        return await _dbContext.Books
            .Include(b => b.Chapters)
                .ThenInclude(c => c.Pages)
    ...
    public async Task<Book?> GetByIdWithChaptersAsync(BookId id, CancellationToken cancellationToken = default)
    {
        return await _dbContext.Books
            .Include(b => b.Chapters)
                .ThenInclude(c => c.Pages)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (genres is not null && genres.Any())
        {
            foreach (var genre in genres)
            {
                ...
                query = query.Where(b =>
                    EF.Property<string>(b, "_genres").ToLower().Contains(genreLower));
            }
        }

        if (languages is not null && languages.Any())
        {
            query = query.Where(b => languages.Contains(b.Metadata.Language));
        }
