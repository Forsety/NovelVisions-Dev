    ...
    : IRequestHandler<ReorderChapterCommand, Result<bool>>
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var sortedChapters = book.Chapters.OrderBy(c => c.OrderIndex).ToList();
            sortedChapters.Remove(chapter);
            sortedChapters.Insert(newIndex - 1, chapter);

            for (var i = 0; i < sortedChapters.Count; i++)
            {
                sortedChapters[i].GetType()
                    .GetProperty(nameof(Domain.Entities.Chapter.OrderIndex))?
                    .SetValue(sortedChapters[i], i + 1);
            }
