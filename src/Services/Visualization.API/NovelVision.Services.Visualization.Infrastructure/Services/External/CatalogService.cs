...
// ИСПРАВЛЕНО: BookApiDto теперь правильно маппит поля HasVisualization и VisualizationSettings
...
public sealed class CatalogService : ICatalogService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var response = await _httpClient.GetAsync(
                $"/api/v1/chapters/{chapterId}?includePages=true",
                cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                var errorContent = await response.Content.ReadAsStringAsync(cancellationToken);
                _logger.LogWarning(
                    "Failed to get book info. Status: {Status}, Error: {Error}",
                    response.StatusCode, errorContent);
            ...
            // ИСПРАВЛЕНО: Правильный маппинг VisualizationEnabled
            // Catalog.API возвращает: hasVisualization (bool) и visualizationSettings.isEnabled (bool)
            // Берём visualizationSettings.isEnabled как основной источник, fallback на hasVisualization
            var visualizationEnabled = apiResponse.Data.VisualizationSettings?.IsEnabled
                                       ?? apiResponse.Data.HasVisualization;

            _logger.LogDebug(
                "Book {BookId} visualization status: HasVisualization={HasVisualization}, " +
                "VisualizationSettings.IsEnabled={IsEnabled}, Computed={Computed}",
                bookId,
                apiResponse.Data.HasVisualization,
                apiResponse.Data.VisualizationSettings?.IsEnabled,
            ...
            var bookInfo = new BookInfoDto
            {
                ...
                Genre = apiResponse.Data.Genres?.FirstOrDefault(),
                VisualizationEnabled = visualizationEnabled,
                PreferredVisualizationStyle = apiResponse.Data.VisualizationSettings?.PreferredStyle,
                TotalPages = apiResponse.Data.PageCount,
                TotalChapters = apiResponse.Data.ChapterCount
            };
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "IsVisualizationEnabled for BookId {BookId}: {Enabled}",
                bookId, bookInfoResult.Value.VisualizationEnabled);
    ...
    /// <summary>
    /// DTO для ответа Catalog.API /api/v1/books/{id}
    /// ИСПРАВЛЕНО: Добавлены HasVisualization, VisualizationSettings, Genres, PageCount, ChapterCount
    /// </summary>
    private sealed record BookApiDto
    {
        ...
        // Catalog.API возвращает массив genres, не одну строку
        public List<string>? Genres { get; init; }

        // Catalog.API возвращает hasVisualization (bool)
        public bool HasVisualization { get; init; }

        // НОВОЕ: Catalog.API возвращает visualizationSettings (объект)
        public VisualizationSettingsApiDto? VisualizationSettings { get; init; }

        // Catalog.API возвращает pageCount и chapterCount, не totalPages/totalChapters
        public int PageCount { get; init; }
        public int ChapterCount { get; init; }

        // Legacy поля на случай если они всё ещё используются где-то
        public int TotalPages { get; init; }
        public int TotalChapters { get; init; }
    }

    /// <summary>
    /// DTO для вложенного объекта visualizationSettings
    /// </summary>
    private sealed record VisualizationSettingsApiDto
    {
        public string? Mode { get; init; }
        public string? PrimaryMode { get; init; }
        ...
        public int MaxImagesPerPage { get; init; }
